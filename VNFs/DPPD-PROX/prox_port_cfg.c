...
static void init_port(struct prox_port_cfg *port_cfg)
{
	...
	if (port_cfg->n_rxq == 0) {
		...
		plog_info("\t\tDriver Name=%s\n", port_cfg->driver_name);
		plog_info("\t\tShort Name=%s\n", port_cfg->short_name);
		// DPDK added a commit specific to virtio NIC: https://github.com/DPDK/dpdk/commit/4e8169eb0d2d34067f56b368633f816ce927650a
		// We need to increase the mbuf_size before requesting rte_mempool_create
		// 498 is the minimum value with which mbuf_size should be increased
		if (strcmp(port_cfg->short_name, "virtio") == 0)
			mbuf_size = port_cfg->mtu+498;
