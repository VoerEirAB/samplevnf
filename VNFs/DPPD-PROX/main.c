...
static void check_missing_rx(void)
{
	...
	while (core_targ_next(&lconf, &targ, 0) == 0) {
		...
		plog_dbg("luuuuuuuuuuuuuuuuuuuuuuuul\n");
		while (core_targ_next(&rx_lconf, &rx_targ, 0) == 0) {
			for (uint8_t i = 0; i < rx_targ->nb_rxports; ++i) {
				plog_dbg("entered this god forsaken for loop once\n");
				rx_port_id = rx_targ->rx_port_queue[i].port;
				plog_dbg("value of rx_port_id: %d and value of port_id: %d");
				if ((rx_port_id == port_id) &&
			...
			plog_dbg("entered this god forsaken while loop once\n");
		}
